package com.embednet.wdluo.bleplatformsdkdemo.util;
...
//          去掉Activity上面的状态栏
             ...
             */
            if (statusBarColor != -1) {
                processStatusBarColor();
            }
        }

        if (hindNavigationBar) {
...
//                        | View.SYSTEM_UI_FLAG_FULLSCREEN //hide statusBar
            ...
            if (navigationBarColor != -1) {
                processNavigationBarColor();
            }
        }

    }

    private void processNavigationBarColor() {
        if (isMoreLollipop()) {
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);

//            Color.TRANSPARENT透明、0x80000000半透明
            window.setNavigationBarColor(navigationBarColor);

        }
    }

    private void processStatusBarColor() {
        if (isMoreLollipop()) {
    ...
    void processLollipopAbove() {
        ...
        if (transparentStatusBar) {
            //改变字体颜色
            flag |= View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_IMMERSIVE | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION;
        }
    ...
    final public static class Builder {
        ...
        private boolean hindNavigationBar = false;
        ...
        public Builder setHindNavigationBar(boolean hindNavigationBar) {
            this.hindNavigationBar = hindNavigationBar;
            return this;
        }

        public void process() {
            new StatusBarUtils(window, lightStatusBar, transparentStatusbar, actionBarView, hindStatusBar, statusBarColor, navigationBarColor, hindNavigationBar).process();
        }
    }
